...
const DbCartManager = require('./DAO/monongoDB/db.carts.js')
...
function initializeSocket(httpServer) {
    ...
    // Variables para almacenar usuarios y mensajes
    const userMessages = {};
...
socket.on('message', message => {
 ...
 // Manejar el evento 'cartData' enviado desde el cliente
 socket.on('cartData', async cartData => {
        console.log('Datos del carrito recibidos:', cartData);

        // Insertar los datos del carrito en la base de datos
        await cart.createDocument(cartData);
    });
