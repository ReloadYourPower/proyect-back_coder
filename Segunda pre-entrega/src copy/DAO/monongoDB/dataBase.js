...
async getProducts(limit , page, order) {
    try {

         // Convertir limit a un número entero
         const limitNumber = parseInt(limit);
         const pageNumber = parseInt(page); // Convertir page a un número entero


         // Verificar si limitNumber es un número válido
         if (isNaN(limitNumber) || limitNumber <= 0 || isNaN(pageNumber) || pageNumber <= 0) {

            // Si alguno de los parámetros no es válido, lanzar un mensaje de advertencia pero continuar con una consulta sin paginación
            console.warn('Limit y page deben ser números enteros positivos. Se devolverán todos los productos.');
         }
        const database = this.client.db(databaseName);
        const collection = database.collection('products');

        // Calculamos el número de documentos a saltar
        const skip = (pageNumber - 1) * limitNumber; // Calcular el número de documentos a omitir

        // Consultar los productos ordenados y paginados
        const cursor = collection.find({})
            .sort({ [order]: 1 }) // Ordenar por el campo especificado
            .skip(skip)
            .limit(limitNumber);

            const totalCount = await cursor.count(); // Contar todos los productos
            const products = await cursor.toArray(); // Obtener los productos de la página actual
    
            const totalPages = Math.ceil(totalCount / limitNumber); // Calcular el número total de páginas
